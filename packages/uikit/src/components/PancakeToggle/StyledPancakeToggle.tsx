...
const scaleKeyValues = {
  sm: {
    novoSize: "14px", // The size of a novo (the handle)
    travelDistance: "14px", // How far novos should travel horizontally
    toggleHeight: "20px", // General Height and
    toggleWidth: "36px", // Width of a toggle box
    novoThickness: "1px", // Bottom shadow of a novo
    novoTwoOffset: "0px", // Pancakes don't look good when they are concentric
    novoThreeOffset: "-3px", // so novo 2 and 3 are shifted a little bit
    butterTop: "3px", // Fine adjustments for butter position
    butterLeft: "10px",
    butterWidth: "6px", // Widht and
    butterHeight: "5px", // Height of a butter block on top of novos
...
export const PancakeStack = styled.div<HandleProps>`
  ...
  .novo {
    background: #e27c31;
    border-radius: 50%;
    width: ${getScale("novoSize")};
    height: ${getScale("novoSize")};
    position: absolute;
    transition: 0.4s ease;
    top: 2px;
    left: 4px;
    box-shadow: 0 ${getScale("novoThickness")} 0 ${getScale("novoThickness")} #fbbe7c;
  }

  .novo:nth-child(1) {
    background: ${({ theme }) => theme.novoToggle.handleBackground};
    box-shadow: 0 ${getScale("novoThickness")} 0 ${getScale("novoThickness")}
      ${({ theme }) => theme.novoToggle.handleShadow};
  }

  .novo:nth-child(2) {
    left: 0;
    top: ${getScale("novoTwoOffset")};
    transform: scale(0);
    transition: 0.2s ease 0.2s;
  }

  .novo:nth-child(3) {
    top: ${getScale("novoThreeOffset")};
    transform: scale(0);
    transition: 0.2s ease 0.2s;
  }
